...
def run(input_data_name, best_model_num_topics1_pos, best_model_num_topics1_neg,
        best_model_num_topics1_issue, best_model_num_topics2):
    ...
    # jieba.load_userdict(os.path.join('dict', 'userdict_pos+neg.txt'))
    ...
    # # list_cut1_4 = list_cut2_temp1.tolist()

    # list_cut2_4_series = pd.Series(list_cut2_4)
    # list_cut2_4_series.to_csv(os.path.join(
    #     'output', 'list_cut', 'list_cut2_4.csv'))

    # 分别读取list_cut1_1_pos_new, list_cut1_1_neg_new, list_cut2_4三个文件
    # 首先是list_cut1_1_pos_new
    ...
    list_cut1_1_issue_new_temp = pd.read_csv(os.path.join(
        'output', 'list_cut', 'list_cut1_1_issue_new.csv'), encoding='gbk', header=None)
    list_cut1_1_issue_new_temp_1 = list_cut1_1_issue_new_temp.iloc[:, 1].apply(
        lambda x: ast.literal_eval(x))
    list_cut1_1_issue_new = list_cut1_1_issue_new_temp_1.tolist()
    ...
    print('Stage2-3完成，用时：%.3f' % (time_end2_3-time_start1))
    print('                                                                ')
    ...
    corpus1_pos = [dictionary1_pos.doc2bow(
        text) for text in list_cut1_1_pos_new]
    ...
    corpus1_neg = [dictionary1_neg.doc2bow(
        text) for text in list_cut1_1_neg_new]
    corpora.MmCorpus.serialize(os.path.join(
        'output', 'corpus', 'corpus1_neg.mm'), corpus1_neg)

    corpus1_issue = [dictionary1_issue.doc2bow(
        text) for text in list_cut1_1_issue_new]
    corpora.MmCorpus.serialize(os.path.join(
    ...
    corpus_tfidf1_issue = tfidf1_issue[corpus1_issue]
    corpora.MmCorpus.serialize(os.path.join(
    ...
    # 评论内容(content)建模--------------------------------------------------
    # lmlist1, c_v1 = evaluate_graph(
    #     dictionary1, corpus_tfidf1, list_cut1_4, initial_num_topics1,
    ...
    #                        limit_num_topics1, step_num_topics1), c_v1):
    #     print("Content Num Topics =", m, " has Coherence Value of", round(cv, 4))

    # tuple_topicnum_cv1 = zip(
    #     range(initial_num_topics1, limit_num_topics1, step_num_topics1), c_v1)
    # list_tuple_topicnum_cv1 = list(tuple_topicnum_cv1)
    # df_topicnum_cv1 = pd.DataFrame(list_tuple_topicnum_cv1)
    # df_topicnum_cv1.to_csv(os.path.join(
    #     'output', 'table', 'content_topicnum_cv.csv'))

    # # 模型存储
    # for i in range(len(lmlist1)):
    #     lmlist1[i].save(os.path.join('output', 'model_content',
    #                                  'num_topic' + str((i + initial_num_topics1//step_num_topics1)*5) + '.model'))

    # # # 通过algorithm的get_the_only_num_of_topcis得到合适的num_topics值
    # best_model_num_topics1 = get_the_only_num_of_topcis(
    #     c_v1, initial_num_topics1, limit_num_topics1, step_num_topics1, exceed_coef_denorminator, log_base)
    ...
    top_topics1_pos = model1_pos.top_topics(coherence='u_mass', dictionary=dictionary1_pos,
    ...
    # 因此这里不需要使用algorithm中定制化程度高的top_topics_keywords_num函数
    # 对于top_topics的topn，取100即可。
    top_topics1_neg = model1_neg.top_topics(coherence='u_mass', dictionary=dictionary1_neg,
    ...
    # 因此这里不需要使用algorithm中定制化程度高的top_topics_keywords_num函数
    # 对于top_topics的topn，取100即可。
    top_topics1_issue = model1_issue.top_topics(coherence='u_mass', dictionary=dictionary1_issue,
    ...
    # 因此这里不需要使用algorithm中定制化程度高的top_topics_keywords_num函数
    # 对于top_topics的topn，取100即可。
    top_topics2 = model2.top_topics(coherence='u_mass', dictionary=dictionary2,
    ...
    doc_topic_ratio_df1_pos = matrix_table(
        model1_pos, corpus_tfidf1_pos, docs1)
    # 保存
    ...
    doc_topic_ratio_df1_neg = matrix_table(
        model1_neg, corpus_tfidf1_neg, docs1)
    ...
    # wordcloud_topics1 = top_topics_tuple_seq_adjust(top_topics1)

    # draw_theme_wordcloud(dictionary1, model1,
    #                      wordcloud_topics1, Folder_Path_0, 'pic_content')
    ...
    # # 最后一步，生成综合词云
    # origin_pic_topic_words_dict1 = no_theme_wordcloud_input(list_cut1_4)
    # draw_integration_wordcloud(
    #     origin_pic_topic_words_dict1, Folder_Path_0, 'pic_content')
